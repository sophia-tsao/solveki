...
def aroc_over_interval(max_degree=5, max_coefficient=10, min_coefficient=-10):
    ...
    constant = random.choice([c for c in range(min_coefficient, max_coefficient+1) if c != 0])
    function += ("+" if constant > 0 else "") + str(constant)
    y_value1 += constant
    y_value2 += constant
    ans=(y_value1-y_value2)/(x_value1-x_value2)
    problem=f'Given the function ${function}$, find the average rate of change over the interval ${x_value1}$<=x<=${x_value2}$'
    solution=f'${ans}$'
    return problem, solution